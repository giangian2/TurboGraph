...
static size_t find_slot(const HashTable *table, const void *key) 
{
    ...
    {
        if (table->state[index] == SLOT_OCCUPIED && memcmp(key_at(table, index), key, table->key_size) == 0) 
        {
            return index;
        }
...
static bool rehash(HashTable *table, size_t new_capacity) 
{
    ...
    for (size_t i = 0; i < old_capacity; i++) 
    {
        ...
        {
            raw_insert(table,
                       (char *)old_keys + i * table->key_size,
...
static bool maybe_grow(HashTable *table) 
{
    ...
    if (load < HT_MAX_LOAD_FACTOR) return true;
...
bool put(HashTable *table, const void *key, const void *value) 
{
    ...
    if (!maybe_grow(table)) return false;
