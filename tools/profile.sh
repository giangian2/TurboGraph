...
# answer on this machine (measured spread on a BFS: +-3 ms out of ~22, while
# the instruction count is exact).
#
...
#   tools/profile.sh -f graph_dfs -c -B               # DFS only, with cache and branches
#   tools/profile.sh -C 2                             # who is calling the hot helpers
#   tools/profile.sh -f graph_dfs -s before           # store the baseline
#   ...edit the code, rebuild...
#   tools/profile.sh -f graph_dfs -d before           # what changed, per function
#
set -euo pipefail

# Always run from the project root: res/ and out/ paths are relative to it.
cd "$(dirname "$0")/.."
...
if ! grep -qE '^==[0-9]+== *Collected *: *[1-9]' "$LOG"; then
    ...
    echo "!! did the program start from the project root and find its res/ inputs?" >&2
    exit 1
fi
